...
class PartPricePredictor:
    ...
    def save_model(self):
        ...
        self._x_train_two_rows.to_parquet(STATIC_DATA_DIR_PATH + model_name + '_x_train_two_rows.parquet')
        with open(STATIC_DATA_DIR_PATH + model_name + ' _model_input_columns.json', 'w') as file:
    ...
    def load_model(self, model_path):
        ...
        # extract model instance from model name, the name has the format model__<list_of_relu_layer_widths_str>__T__<training_table_name>
    ...
    def price_predictions_for_all_feature_combinations(self, all_tables_df, csv_filename=None):
        ...
        ret_df['pred_price'] = ret_df.apply(lambda row: self.predict_part_price({k: [v] for k, v in row.to_dict().items()}), axis=1)
    ...
    def complete_columns_with_negatives(self, prepared_data):
        if self._x_train_two_rows is not None:
            ...
            if len(missing_columns) != len(missing_columns_1) or len(set(missing_columns).symmetric_difference(set(missing_columns_1))) > 0:
